...
export default function CardiacMetrics({ daily }: Props) {
  const vitals = [
    ...
    { label: 'HRV',                    main:'55 ms',   sub: 'SDNN avg · peak 71 ms',          good: true,  values: daily.map(d => Math.round(d.hrv_ms)),          max: 100, color: 'var(--accent-green)' },
    ...
    { label: 'Walking HR Avg',       main: '107 bpm', sub: 'includes sprints/jogs',          good: false, values: daily.map(d => Math.round(d.walking_heart_rate)),max:160, color: 'var(--accent-coral)' },
    { label: 'Resp Rate',             main: '14.8 br/m',sub:'sleep avg · range 13.5–16.3',   good: true,  values: daily.map(d => d.respiratory_rate) as (number|null)[], max:20, color:'var(--accent-green)' },
    { label: 'Wrist Temp',           main: '95.4 °F', sub: 'sleep baseline',                good: false, values: daily.map(d => d.wrist_temp_c ? +(d.wrist_temp_c * 9/5 + 32).toFixed(1) : null),  max: 97,  color: 'var(--accent-coral)' },
  ]
